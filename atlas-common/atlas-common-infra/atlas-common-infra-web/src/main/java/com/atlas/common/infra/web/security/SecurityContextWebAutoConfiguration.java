 ...
 * 安全上下文 Web 自动配置
 *
 * <p>注册 SecurityContextImpl、SecurityContextFilter，并将 SecurityContext 注册到
 * SecurityContextHolder，使仅依赖 common 的业务服务（不依赖 atlas-auth）也能通过 Gateway 下传请求头获得
 * 当前用户与审计填充。当未提供 {@link TokenValidator} 时使用仅返回 null 的默认实现，仅从请求头解析用户。
 *
 * @author Atlas
 * @since 1.0.0
 */
