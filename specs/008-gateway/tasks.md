...
- [ ] T096 [US6] 完善 `GatewayProperties` 配置属性类，确保所有配置项都支持动态更新
- [ ] T097 [US6] 实现配置变更监听器，监听 Nacos Config 配置变更事件
- [ ] T098 [US6] 实现路由规则动态更新功能（配置变更后重新加载路由规则）
- [ ] T099 [US6] 实现白名单动态更新功能（配置变更后重新加载白名单配置）
- [ ] T100 [US6] 实现 CORS 配置动态更新功能（配置变更后重新加载 CORS 配置）
- [ ] T101 [US6] 验证配置项符合项目的配置命名规范（`atlas.gateway.*`）
- [ ] T102 [US6] 创建配置动态更新集成测试，验证所有配置都可以动态更新
