...
- [ ] T006 [US1] 在项目根目录执行 `mvn clean install` 命令
- [ ] T007 [US1] 检查构建日志，确认无编译错误
- [ ] T008 [US1] 检查构建日志，确认无版本不兼容错误
- [ ] T009 [US1] 检查构建日志，确认无依赖冲突错误
- [ ] T010 [US1] 验证构建退出码为 0
- [ ] T011 [US1] 验证所有模块编译成功（检查构建日志中的模块列表）
- [ ] T012 [US1] 验证构建产物（JAR 文件）在 `target/` 目录下正常生成
- [ ] T013 [US1] 如果构建失败，记录错误信息到验证报告
